	...
	<div class="container-fluid">
		...
		<div>
			<%
				if(currentPage > 1){
			%>
			<a href="<%=request.getContextPath()%>/tableList/nicerButSlowerFilmList.jsp?currentPage=<%=currentPage-1 %>">이전</a>	
			<%
				}
			%>
			<%
			...
			%>
			<a href="<%=request.getContextPath()%>/tableList/nicerButSlowerFilmList.jsp?currentPage=<%=currentPage + 1 %>">다음</a>	
			<%
				}
			%>
		</div>
